...
if [ -f "${GOPATH}/bin/taskcluster-worker" ]; then
  mkdir "${GOPATH}/bin/linux_amd64"
  mv "${GOPATH}/bin/taskcluster-worker" "${GOPATH}/bin/linux_amd64/taskcluster-worker"
fi
...
export RELEASE_FILE="${TRAVIS_BUILD_DIR}/taskcluster-worker-${GIMME_OS}-${GIMME_ARCH}${FILE_EXT}"
mv "${GOPATH}/bin/${GIMME_OS}_${GIMME_ARCH}/taskcluster-worker${FILE_EXT}" "${RELEASE_FILE}"
